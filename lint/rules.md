...
| 20 | Ungoverned system file | HIGH | `python3 scripts/check_governance.py --vault ~/wiki` (exit 1). Every prose file at the root of `_system/` is declared in `_system/governance.yaml` as `governs`, `reference` or `dead`; every declaration still has a file; and every `governs` entry's `reader` still mentions it. The third check is the one with teeth — a governing document whose last reader disappeared still looks authoritative and nothing else notices. `_system/` is outside the page schema by design, so rules 1-17 never see it: this is the only rule that does. Unverifiable (the reader path itself is gone) is reported, NOT graded | Declare the new file, delete the stale declaration, or re-home the orphaned document — one deposit intent per finding |
| 21 | Undrained inbox | HIGH | `python3 scripts/check_inbox.py --vault ~/wiki` (exit 1). Every file at the capture inbox root is named somewhere in the task registry, or is younger than the grace period (14d). Deliberately weak: it does not care whether the linked task is open or closed, only whether ANYTHING in the system knows the file exists. A file captured this morning SHOULD be unlinked — one unlinked after two weeks was never triaged. Age comes from `git log`, never mtime, which a checkout rewrites. Untracked-by-git is reported, NOT graded | Link it, delete it, or move it out of the inbox. Leave-it is not one of the three outcomes |
